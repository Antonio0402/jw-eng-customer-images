...
if ( ! class_exists( __NAMESPACE__ . '\\' . 'Router' ) ) {
	 ...
	 * @subpackage JW_Eng_Customer_Images/controllers
	 */
	class Router {
		...
		public function generic_route_types() {
			return apply_filters(
				'jw_eng_customer_images_route_types', [
					Route_Type::ANY,
					Route_Type::ADMIN,
		...
		public function late_frontend_route_types() {
			return apply_filters(
				'jw_eng_customer_images_late_frontend_route_types', [
		...
		public function register_route_of_type( $type ) {
			if ( in_array( $type, $this->late_frontend_route_types() ) && did_action( 'wp' ) ) {
				trigger_error( __( 'Late Routes can not be registered after `wp` hook is triggered. Register your route before `wp` hook is triggered.', JW_Eng_Customer_Images::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}

			if ( in_array( $type, $this->generic_route_types() ) && did_action( 'init' ) ) {
				trigger_error( __( 'Non-Late Routes can not be registered after `init` hook is triggered. Register your route before `init` hook is triggered.', JW_Eng_Customer_Images::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
		...
		private function get_fully_qualified_class_name( $class, $mvc_component_type, $route_type ) {
			...
			if ( \strpos( $route_type, 'admin' ) !== false || \strpos( $route_type, 'frontend' ) !== false ) {
				$fqcn = '\JW_Eng_Customer_Images\App\\';
