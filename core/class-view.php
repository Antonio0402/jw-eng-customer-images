 ...
 * @subpackage JW_Eng_Customer_Images/views
...
class View {
	...
	public static function render_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		ob_start();
		do_action( 'jw_eng_customer_images_before_template_render', $template_name, $template_path, $located, $args );
		include( $located );
		do_action( 'jw_eng_customer_images_after_template_render', $template_name, $template_path, $located, $args );
	...
	public static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
		if ( ! $template_path ) {
			$template_path = 'jw-eng-customer-images-templates/';
		}
		if ( ! $default_path ) {
			$default_path = JW_Eng_Customer_Images::get_plugin_path() . 'app/templates/';
		}
		...
		if ( file_exists( $template ) ) {
			...
			return apply_filters( 'jw_eng_customer_images_locate_template', $template, $template_name, $template_path );
		} else {
			return false;
		}
	}
