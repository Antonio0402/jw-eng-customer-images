...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	 ...
	 * @subpackage JW_Eng_Customer_Images/Includes
	 */
	class Requirements_Checker {
		 ...
		 *
		 * @param string $jw_eng_customer_images Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $jw_eng_customer_images, $plugin_slug ) {
			...
			$this->add_error_notice(
				$jw_eng_customer_images . ' is a required plugin.',
				$jw_eng_customer_images . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $jw_eng_customer_images JW Eng Customer Images.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $jw_eng_customer_images, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$jw_eng_customer_images} {$min_plugin_version}+ is required.",
				"{$jw_eng_customer_images} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_jw_eng_customer_images ) {
					return $this->is_plugin_active( $required_jw_eng_customer_images, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_jw_eng_customer_images ) {
					return $this->is_required_plugin_version_active( $required_jw_eng_customer_images, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
